...
public:
    string reorderSpaces(string text) {
        ...
        //got words in vector
        //got space count 
        //divide spaces among words and find the space at end as well
        int spacesBetw = spaceCount/(arr.size()-1);
        int spacesleft = spaceCount - spacesBetw*(arr.size()-1);

        for(int i=0;i<arr.size();i++){
            buffer+=arr[i];
            spacesBetw = spaceCount/(arr.size()-1);
            if(i == arr.size()-1){
                while(spacesleft>0){
                buffer+=" ";
                spacesleft--;
